 ...
 *
 *
 * Edits: Added the feature that an experiment could be run that includes both a trial limit and a maxNumSteps limit.
 * note that generality was kept here, and it could be a desired feature enhancement (and thus in-line for a pr)
 */
public class BoundedLearningAlgorithmExperimenter {
	...
	/**
	 * This holds the maximum allowable steps per episode when both episode limits, and step limits are imposed
	 */
	protected int 						maxNumSteps;

	/**
	 * This indicates whether or not episodes are bounded by maxNumSteps
	 */
	protected boolean 					stepLimited = false;
	 ...
	 */
	public BoundedLearningAlgorithmExperimenter(SADomain domain, RewardFunction rf, StateGenerator sg, int nTrials, int trialLength, LearningAgentFactory...agentFactories){
	...
	/**
	 * Initializes.
	 * The trialLength will be interpreted as the number of episodes; it can be reinterpreted as a total number of steps per trial using the
	 * {@link #toggleTrialLengthInterpretation(boolean)} as before, but with the standard interpretation, episodes are bounded by maxSteps.
	 * @param domain the domain in which agents will be tested
	 * @param rf the reward function used to measure performance
	 * @param sg the state generated used to generate states at the beginning of each episode
	 * @param nTrials the number of trials
	 * @param maxStepsEpisode the maximum steps per episode when running episodic trials
	 * @param trialLength the length of the trials (by default in episodes, but can be intereted as maximum step length)
	 * @param agentFactories factories to generate the agents to be tested.
	 */
	public BoundedLearningAlgorithmExperimenter(SADomain domain, RewardFunction rf, StateGenerator sg, int nTrials, int trialLength,
												 int maxStepsEpisode, LearningAgentFactory...agentFactories){
		
		this(domain, rf, sg, nTrials, trialLength, agentFactories);
		this.maxNumSteps = maxStepsEpisode;
		this.stepLimited = true;
	}
	...
	protected void runEpisodeBoundTrial(LearningAgentFactory agentFactory){
		...
		for(int i = 0; i < this.trialLength; i++){
			if(stepLimited)
				agent.runLearningEpisodeFrom(sg.generateState(), maxNumSteps);
			else
