 ...
 *
 *
 * Note: This file has been edited for use in the RS experiments. 
 * Edits by Maxwell J Svetlik
 */

public class RSGridWorld implements DomainGenerator {

	/*
	* Project resources use this values to determine debug output.
	*/
	public static final boolean 						DEBUG = true;
	/**
	 * Defines possible methods for choosing &/ merging potential function values
	 * Possible options: 'max' 'min' and 'average'
	 */
	public static final String 							HEURISTIC = "average";
	/**
	...
	/**
	 * Constant for the name of the name attribute used for obstacles
	 */
	public static final String							ATTNAME = "name";

	public static final String 							ATTINSTANCE = "instance";
	/**
	 ...
	 * Obstacle constants
	 */
	public static final String 							OBSTACLEROCK = "rock";

	public static final String 							CLASSOBSTACLE = "obstacle";

	public static final String 							OBSTACLEPIT = "pit";

	public static final String 							OBSTACLEFIRE = "fire";

	public static final String 							OBSTACLEMUSIC = "music";

	public static final String 							OBSTACLENULL  = "dummy";

	public static final String 							OBSTACLEGOAL = "goal";
	...
	public static final String 							PFNEVERTERM = "neverTerm";
	/**
	...
	/**
	 * Constant for obstacle placement. In order for offline visualization and reward shaping, all possible obstacle names are listed here.
	 * Note however that these will be dynamically altered by the constructor(s). Thus, you must pass in the string[] of obstacles desired.
	 */
	public static String[]						OBSTACLECLASSNAMES = {OBSTACLEFIRE, OBSTACLEPIT, OBSTACLEMUSIC};

	/**
	 * Constant for the number of obstacles. In order for offline visualization, unfortunately this must be an in-class constant.
	 */	
	public static int 							NUMOBSTACLES = 4;
	...
	protected ArrayList<RSGridWorldRewardFunction.IntPair>						locationAccess;
	 ...
	 */
	public RSGridWorld(int width, int height, String[] obstacleNames, int numObstacles){
		this.width = width;
		this.height = height;
		this.setDeterministicTransitionDynamics();
		this.makeEmptyMap();
		this.NUMOBSTACLES = numObstacles;
		this.OBSTACLECLASSNAMES = obstacleNames;
		locationAccess = new ArrayList<RSGridWorldRewardFunction.IntPair>();
	}
	 ...
	 */
	public RSGridWorld(int [][] map){
		this.setMap(map);
		this.setDeterministicTransitionDynamics();
	}
	
	/*
	 * This method allows one to track the states accessed by the agent.
	 * Returns true if the passed location has already been accessed, false if its a new experience.
	 */
	public boolean addLocation(RSGridWorldRewardFunction.IntPair accessed){
		if(!locationAccess.contains(accessed)){
			locationAccess.add(accessed);
			return false;
		}
		return true;
	}
	...
	public Domain generateDomain() {
		...
		Attribute nameatt = new Attribute(domain, ATTNAME, Attribute.AttributeType.STRING);
		
		Attribute ltatt = new Attribute(domain, ATTLOCTYPE, Attribute.AttributeType.INT);
		ltatt.setDiscValuesForRange(0, numLocationTypes-1, 1);
		
		Attribute instanceatt = new Attribute(domain, ATTINSTANCE, Attribute.AttributeType.INT);
		...
		ObjectClass obstacleClass = new ObjectClass(domain, CLASSOBSTACLE);
		obstacleClass.addAttribute(xatt);
		obstacleClass.addAttribute(yatt);
		obstacleClass.addAttribute(nameatt);
		obstacleClass.addAttribute(instanceatt);
		...
		new NeverTermPF(PFNEVERTERM, domain, new String[]{CLASSAGENT, CLASSLOCATION});
	...
	/**Returns a state object with n obstacle objects added of a type specified by the class constant*/
	public static State getOneAgentOneLocationNObstacleState(Domain d, int n){
		State s = new State();
		
		//create n class instances for each obstacle type
		for(int j = 0; j < OBSTACLECLASSNAMES.length * n; j++){
			s.addObject(new ObjectInstance(d.getObjectClass(CLASSOBSTACLE), CLASSOBSTACLE+j));
		}
		s.addObject(new ObjectInstance(d.getObjectClass(CLASSLOCATION), CLASSLOCATION+0));
		s.addObject(new ObjectInstance(d.getObjectClass(CLASSAGENT), CLASSAGENT+0));
	...
	/*
	*Returns a state object with n obstacle objects added of a type specified by the class constant
	*No locations are added here
	*/
	public static State getOneAgentNObstacleState(Domain d, int n){
		State s = new State();
		...
		/**Returns a state object with *no* obstacle objects of types specified by the class constant.
		 * and nl objects of type CLASSLOCATION
		 */
	public static State getOneAgentNLocationNObstacleState(Domain d, int nl, int no){
		State s = new State();
		
		for(int i = 0; i < nl; i++){
			s.addObject(new ObjectInstance(d.getObjectClass(CLASSLOCATION), CLASSLOCATION+i));
			s.addObject(new ObjectInstance(d.getObjectClass(CLASSOBSTACLE),CLASSOBSTACLE+0));
		}
		s.addObject(new ObjectInstance(d.getObjectClass(CLASSAGENT), CLASSAGENT+0));

		for(int j = 0; j < OBSTACLECLASSNAMES.length * no; j++){
			s.addObject(new ObjectInstance(d.getObjectClass(CLASSOBSTACLE), CLASSOBSTACLE+j));
		}
	...
	/**Sets an obstacle of specified class name to an x and y position, as well as the name attribute for the obstacle.
		Currently there is no check if the class exists or not, and assumes that it has x, y, and name attributes.
		*/
	public static void setObstacle(State s, int i, int x, int y, String className){
		ObjectInstance o = s.getObjectsOfClass(CLASSOBSTACLE).get(i);

		o.setValue(ATTX, x);
		o.setValue(ATTY, y);
		o.setValue(ATTNAME, className);
	}

	public static void setObstacle(State s, int obx, int oby, String obClass, int setx, int sety, String setClass){
		ObjectInstance agent = s.getObjectsOfClass(RSGridWorld.CLASSAGENT).get(0);
		int ax = agent.getIntValForAttribute(RSGridWorld.ATTX);
		int ay = agent.getIntValForAttribute(RSGridWorld.ATTY);

		for( ObjectInstance ob : s.getObjectsOfClass(RSGridWorld.CLASSOBSTACLE)){
			if(ob.getStringValForAttribute(RSGridWorld.ATTNAME).equals(obClass)){
				int fx = ob.getIntValForAttribute(ATTX);
				int fy = ob.getIntValForAttribute(ATTY);
				if(fx == ax && fy == ay){
					ob.setValue(ATTX,setx);
					ob.setValue(ATTY,sety);
					ob.setValue(ATTNAME,setClass);
				}
			}
		}
	}
	...
	public static void setLocation(State s, int i, int x, int y){
		...
		//ObjectInstance goal = s.getObjectsOfClass(CLASSOBSTACLE).get(); //gets the 'goal obstacle'
		//goal obstacle has no real meaning beyond reference for pot functions. 0 is a hardcoded value
		//goal.setValue(ATTX,x);
		//goal.setValue(ATTY,y);
		//goal.setValue(ATTNAME, OBSTACLEGOAL);
		o.setValue(ATTX, x);
		o.setValue(ATTY, y);
	...
	protected void move(State s, int xd, int yd, int [][] map){
		...
		//due to an instantiation timing issue, rocks cannot be implemented as walls
		//so check for them here. if hit rock, don't move
		/*for( ObjectInstance ob : s.getObjectsOfClass(CLASSOBSTACLE)){
			if(ob.getIntValForAttribute(ATTX) == nx && ob.getIntValForAttribute(ATTY) == ny && ob.getStringValForAttribute(ATTNAME).equals(OBSTACLEROCK)){
				nx = ax;
				ny = ay;
				//check if agent is in a pit: if so, movement is not possible 
			} else if(ob.getIntValForAttribute(ATTX) == ax && ob.getIntValForAttribute(ATTY) == ay && ob.getStringValForAttribute(ATTNAME).equals(OBSTACLEPIT)){
				nx = ax;
				ny = ay;
			}
		}*/
	...
	public class MovementAction extends Action{
		...
		protected State performActionHelper(State st, String[] params) {
			...
			int [] dcomps = RSGridWorld.this.movementDirectionFromIndex(dir);
			RSGridWorld.this.move(st, dcomps[0], dcomps[1], this.map);
			return st;
		}
		...
		public List<TransitionProbability> getTransitions(State st, String [] params){
			...
			for(int i = 0; i < directionProbs.length; i++){
				...
				State ns = st.copy();
				int [] dcomps = RSGridWorld.this.movementDirectionFromIndex(i);
				RSGridWorld.this.move(ns, dcomps[0], dcomps[1], this.map);
	 ...
	 * @author James MacGlashan
	 *
	 * Edited to include states in which the agent is in an OBSTACLEPIT as terminiating
	 */
	public class AtLocationPF extends PropositionalFunction{
		...
		public boolean isTrue(State st, String[] params) {
			...
			for(ObjectInstance l : obst){
				if(l.getStringValForAttribute(ATTNAME).equals(RSGridWorld.OBSTACLEPIT)){
					if(l.getIntValForAttribute(ATTX) == ax && l.getIntValForAttribute(ATTY) == ay){
						return false;
					}
				}
			}
	...
	public class NeverTermPF extends PropositionalFunction{
		/**
		 * Initializes with given name domain and parameter object class types
		 * @param name name of function
		 * @param domain the domain of the function
		 * @param parameterClasses the object class types for the parameters
		 */
		public NeverTermPF(String name, Domain domain, String[] parameterClasses) {
			super(name, domain, parameterClasses);
		}
		@Override
		public boolean isTrue(State st, String[] params) {
	...
	public class WallToPF extends PropositionalFunction{
		...
		public WallToPF(String name, Domain domain, String[] parameterClasses, int direction) {
			super(name, domain, parameterClasses);
			int [] dcomps = RSGridWorld.this.movementDirectionFromIndex(direction);
			xdelta = dcomps[0];
			ydelta = dcomps[1];
		}
		...
		public boolean isTrue(State st, String[] params) {
			...
			if(cx < 0 || cx >= RSGridWorld.this.width || cy < 0 || cy >= RSGridWorld.this.height || RSGridWorld.this.map[cx][cy] == 1 || 
					(xdelta > 0 && (RSGridWorld.this.map[ax][ay] == 3 || RSGridWorld.this.map[ax][ay] == 4)) || (xdelta < 0 && (RSGridWorld.this.map[cx][cy] == 3 || RSGridWorld.this.map[cx][cy] == 4)) ||
					(ydelta > 0 && (RSGridWorld.this.map[ax][ay] == 2 || RSGridWorld.this.map[ax][ay] == 4)) || (ydelta < 0 && (RSGridWorld.this.map[cx][cy] == 2 || RSGridWorld.this.map[cx][cy] == 4)) ){
				return true;
			}
			return false;
		}
