 ...
 *
 * Note: This has been edited for visualization of different types of class Objects that wasn't able to be applied directly.
 * At this time, there hasn't been an effort to keep generality. (ie, works for the RS experiments only)
 * Edits by Maxwell J Svetlik
 *
 */
public class RSGridWorldVisualizer {
	...
	public static StateRenderLayer getRenderLayer(Domain d, int [][] map){
		...
		r.addObjectClassPainter(RSGridWorld.CLASSLOCATION, new LocationPainter(map));
		r.addObjectClassPainter(RSGridWorld.CLASSOBSTACLE, new ObstaclePainter(map));
		r.addObjectClassPainter(RSGridWorld.CLASSAGENT, new CellPainter(1, Color.green, map));
		return r;
	...
	public static class CellPainter implements ObjectPainter{
		...
		public void paintObject(Graphics2D g2, State s, ObjectInstance ob, float cWidth, float cHeight) {
			...
			float rx = ob.getIntValForAttribute(RSGridWorld.ATTX)*width;
			float ry = cHeight - height - ob.getIntValForAttribute(RSGridWorld.ATTY)*height;
	...
	public static class LocationPainter implements ObjectPainter{
		...
		public LocationPainter(int [][] map) {
			...
			this.dheight = map[0].length;
			this.map = map;
			this.baseColors = new ArrayList<Color>(1);
		...
		public void paintObject(Graphics2D g2, State s, ObjectInstance ob, float cWidth, float cHeight) {
			...
			//set the color of the object
			if(ob.getTrueClassName().contains("location")){
				int type = ob.getIntValForAttribute(RSGridWorld.ATTLOCTYPE);
			...
			else if(ob.getTrueClassName().contains("rock"))
				g2.setColor(Color.GRAY);
			else if (ob.getTrueClassName().contains("fire"))
				g2.setColor(Color.RED);
			else if (ob.getTrueClassName().contains("music"))
		 ...
		 * @param map the wall map matrix where 1s indicate a wall in that cell and 0s indicate it is clear of walls
		 */
		public ObstaclePainter(int [][] map) {
			this.dwidth = map.length;
			this.dheight = map[0].length;
			this.map = map;
		}
		...
		public void paintObject(Graphics2D g2, State s, ObjectInstance ob, float cWidth, float cHeight) {
			//set the color of the object
			if(ob.getStringValForAttribute(RSGridWorld.ATTNAME).equals(RSGridWorld.OBSTACLEROCK))
				g2.setColor(Color.GRAY);
			else if (ob.getStringValForAttribute(RSGridWorld.ATTNAME).equals(RSGridWorld.OBSTACLEFIRE))
				g2.setColor(Color.RED);
			else if (ob.getStringValForAttribute(RSGridWorld.ATTNAME).equals(RSGridWorld.OBSTACLEPIT))
				g2.setColor(new Color(156, 93, 82)); //brown
			else if (ob.getStringValForAttribute(RSGridWorld.ATTNAME).equals(RSGridWorld.OBSTACLEMUSIC))
